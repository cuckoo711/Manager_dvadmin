...
class MessageCenterViewSet(CustomModelViewSet):
    ...
    @action(methods=['GET'], detail=False, permission_classes=[IsAuthenticated])
    def read_all_msg(self, request):
        self_user_id = self.request.user.id
        queryset = MessageCenterTargetUser.objects.filter(users__id=self_user_id, is_read=False)
        if queryset:
            queryset.update(is_read=True)
            return DetailResponse(msg="操作成功", data={"status": True})
        else:
            return DetailResponse(msg="操作成功", data={"status": False})
