...
class Menu(CoreModel):
    ...
    @classmethod
    def get_all_parent(cls, _id: int, all_list=None, nodes=None):
        """
        递归获取给定ID的所有层级
        :param _id: 参数ID
        :param all_list: 所有列表
...
class DownloadCenter(CoreModel):
    ...
    task_status = models.SmallIntegerField(default=0, choices=TASK_STATUS_CHOICES, verbose_name='是否可下载',
                                           help_text='是否可下载')
    ...
    def is_expired(self):
        if self.task_status == 2 and (self.create_datetime < (datetime.now() - timedelta(days=3))):
            return True
        return False
