...
if [ -f "shs_2/local_merge_single_3.sh" ]; then
  echo "Executing shs_2/local_merge_single_3.sh..."
  bash shs_2/local_merge_single_3.sh
  if [ $? -ne 0 ]; then
    echo "Error occurred while executing shs_2/local_merge_single_3.sh"
    exit 1
  fi
  echo "shs_2/local_merge_single_3.sh executed successfully!"
else
  echo "shs_2/local_merge_single_3.sh not found, skipping..."
fi
