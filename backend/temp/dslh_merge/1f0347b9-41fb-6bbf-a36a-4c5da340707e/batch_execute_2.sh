...
if [ -f "shs_2/local_merge_single_5.sh" ]; then
  echo "Executing shs_2/local_merge_single_5.sh..."
  bash shs_2/local_merge_single_5.sh
  if [ $? -ne 0 ]; then
    echo "Error occurred while executing shs_2/local_merge_single_5.sh"
    exit 1
  fi
  echo "shs_2/local_merge_single_5.sh executed successfully!"
else
  echo "shs_2/local_merge_single_5.sh not found, skipping..."
fi
