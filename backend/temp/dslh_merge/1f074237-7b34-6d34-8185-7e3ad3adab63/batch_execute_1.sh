...
if [ -f "shs_1/local_merge_single_4.sh" ]; then
  echo "Executing shs_1/local_merge_single_4.sh..."
  bash shs_1/local_merge_single_4.sh
  if [ $? -ne 0 ]; then
    echo "Error occurred while executing shs_1/local_merge_single_4.sh"
    exit 1
  fi
  echo "shs_1/local_merge_single_4.sh executed successfully!"
else
  echo "shs_1/local_merge_single_4.sh not found, skipping..."
fi
