...
def async_updload_map(header, datas, *args, **kwargs):
    ...
    for channel_column in header:
        channel_obj = ServiceTableChannel.objects.filter(column=channel_column).first()
        if not channel_obj:
            continue
        channel_temp[channel_column] = channel_obj

    for index, row in datas:
        for i in range(len(header)):
            channel_column = header[i]
            if (
                    not channel_temp.get(channel_column) or
                    not row[channel_column] or
                    row[channel_column] == 'nan' or
                    pd.isnull(row[channel_column])
            ):
                continue
            new_datas.append(
                ServiceTableMap(
                    game_name=row['游戏名'],
                    channel=channel_temp[channel_column],
                    game_map_name=row[channel_column]
                )
            )
    with transaction.atomic():
        ServiceTableMap.objects.bulk_create(new_datas)
    return {"message": "上传成功", "count": len(new_datas)}
...
def task__generate_service_split_table(split_service_table_id: int, *args, **kwargs):
    ...
    try:
        ...
        for template in templates_splits:
            ...
            for gamename, datas in service_table_dict.items():
                ...
                if template.output_engine == '0':
                    write_to_excelwriter(template, datas, output_file)
                    ...
                    write_to_workbook(template, datas, output_file)
                else:
        ...
        for template in templates_no_splits:
            ...
            if template.output_engine == '0':
                write_to_excelwriter(template, datas, no_split_output_path)
            elif template.output_engine == '1':
                res_data = pd.DataFrame(eval(template.template_fields))
                if template.output_format == '2':
                ...
                write_to_workbook(template, datas, no_split_output_path)
            else:
                logs.append(f"{template.template_name}未配置输出引擎")
        ...
        zip_dir_by_linux(output_path, output_zip)
        ...
        logs.append(f"生成失败, 错误: {str(e)}")
        split.generate_status = '2'
        split.generate_logs = '\n'.join(logs)
        split.save()
        return {"error": f"生成失败, 错误: {str(e)}"}
    return {"message": "生成成功", "logs": logs}


def zip_dir_by_linux(output_path: str, output_zip: str):
...
def write_to_excelwriter(template: ServiceTableTemplate, datas: list, output_path: str):
    field, headers = str(template.template_fields).split('||')
    fields = field.replace('&&', ' ').split(',')
    headers = headers.split(',')

    if template.output_format == '0':
                ...
                for key, value in data.items():
                    if field.startswith(key):
                        if 'open_datetime' in field:
                            ...
                            elif datetime_type.startswith('numformat_'):
                                work_sheet.write_datetime(
                                    index + 1, col, data[key].to_pydatetime(),
    ...
    if template.output_format == '0':
        Changer.change_xlsx_to_xls(output_path)


def write_to_workbook(template: ServiceTableTemplate, datas: list, output_path: str):
    field, headers = str(template.template_fields).split('||')
    fields = field.replace('&&', ' ').split(',')
    headers = headers.split(',')
        ...
        for col, field in enumerate(fields):
            for key, value in data.items():
                if field.startswith(key):
                    ...
                    elif 'open_datetime' in field:
                        datetime_type = field.split('open_datetime_')[-1]
                        if datetime_type.startswith('strftime_'):
                            if '_str_' in datetime_type:
                                datetime_format = datetime_type.split('strftime_')[-1].split('_str_')[0]
                                str_format = datetime_type.split('_str_')[-1]
                                datastyle.num_format_str = str_format
                                worksheet.write(index + 1, col, data[key].strftime(datetime_format), datastyle)
                            else:
                                datetime_format = datetime_type.split('strftime_')[-1]
                                worksheet.write(index + 1, col, data[key].strftime(datetime_format))
                        else:
