...
class QuickDetail:
    ...
    def use_revenue_split_temp(self, channel, game) -> RevenueSplit:
        return self.revenue_split_temp.get(f"{channel}-{game}", self.check_revenue_split(channel, game))

    def check_revenue_split(self, channel, game) -> RevenueSplit:
    ...
    def use_research_split_temp(self, game) -> ResearchSplit:
        return self.research_split_temp.get(game, self.check_research_split(game))

    def check_research_split(self, game) -> ResearchSplit:
    ...
    def _process_item(self, item, message) -> bool:
        ...
        if not channel_obj:
            self._add_message(
                message, 'warning', f"*[{item['channel']}({channel}): {item['game']}] 未找到渠道信息, 请检查")
            return False
        ...
        if not game_obj:
            self._add_message(
                message, 'warning', f"*[{item['channel']}({channel}): {item['game']}] 未找到游戏信息, 请检查")
            return False
        ...
        if not revenue_split_obj:
            self._add_message(
                message, 'warning', f"*[{item['channel']}({channel}): {item['game']}] 未找到渠道分成信息, 请检查")
            return False

        research_split_obj = self.use_research_split_temp(game_obj.id)
        if not research_split_obj:
            self._add_message(
                message, 'warning', f"*[{item['channel']}({channel}): {item['game']}] 未找到研发分成信息, 请检查")
            return False
        ...
        for data in datas:
            ...
            for child in data['children']:
                child_temp = [
                    ...
                    Decimal(child.get('research_folding_income', 0) if child.get(
                        'research_folding_income') != '信息缺失' else 0),
