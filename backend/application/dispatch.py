        ...
        if is_tenants_mode():
            ...
            for tenant in get_tenant_model().objects.filter():
                with tenant_context(tenant):
                    # 假设 _get_all_dictionary() 可能返回 None 或符合预期类型
                    dictionary_data = _get_all_dictionary()

                    # 如果 dictionary_data 为 None，处理为一个默认值，避免类型不一致
                    if dictionary_data is None:
                        dictionary_data = {}

                    # 确保返回的数据结构符合预期类型
                    if isinstance(dictionary_data, dict):
                        settings.DICTIONARY_CONFIG[connection.tenant.schema_name] = dictionary_data
                    else:
                        raise ValueError("Expected dictionary from _get_all_dictionary, got {type(dictionary_data)}")
