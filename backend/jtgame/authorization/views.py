...
class NoticeSerializer(CustomModelSerializer):
    game_names = serializers.SerializerMethodField()
...
class NoticeViewSet(CustomModelViewSet):
    ...
    def download(self, request, pk=None):
        try:
            obj = self.get_object()
            if not obj.notice_filepath:
                return JsonResponse({'status': False, 'message': '通知文件未生成，请先生成'})

            file_path = obj.notice_filepath
            if not os.path.exists(file_path):
                return JsonResponse({'status': False, 'message': '文件未找到'})

            response = FileResponse(open(file_path, 'rb'), content_type='application/zip')
            response['Content-Disposition'] = f'attachment; filename="{obj.title}.zip"'
            return response
        except Exception as e:
            return JsonResponse({'status': False, 'message': str(e)})
    ...
    @action(detail=False, methods=['get'])
    def clear(self, request, pk=None):
        if Notice.objects.filter(status=2).exists():
            return JsonResponse({'status': False, 'message': '存在正在生成的通知文件，请稍后再试'})
        try:
            objs = Notice.objects.filter(notice_filepath__isnull=False).values_list(
                'notice_filepath', flat=True)
            save_path = set(objs)

            output_dir_config = AuthorizationConfig.objects.filter(key='notice_output_dir').first()
            if not output_dir_config:
                return JsonResponse({'status': False, 'message': '未找到通知文件输出路径配置'}, status=400)
            output_dir = output_dir_config.value
