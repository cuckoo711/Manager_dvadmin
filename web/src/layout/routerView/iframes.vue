    ...
    <div class="layout-padding-auto layout-padding-view">
      <div class="w100" v-for="v in setIframeList" :key="v.path" v-loading="v.meta.loading"
        ...
        <transition-group :name="name" mode="out-in">
          <iframe
              :src="getProxyUrl(v.meta.isLink)"
              :key="v.path"
              frameborder="0"
              sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-top-navigation-by-user-activation
        allow-popups-to-escape-sandbox allow-presentation allow-modals allow-orientation-lock allow-pointer-lock"
              height="100%"
              width="100%"
...
const getProxyUrl = (url: any) => {
  // 使用代理服务器，拼接请求参数
  // return `https://gamemanageproxy.jingtanggame.com/proxy?url=${encodeURIComponent(url)}`;
  return url;
};
