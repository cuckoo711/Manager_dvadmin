...
export function showUpgrade() {
    if (Session.get(IS_SHOW_UPGRADE_SESSION_KEY) === true) {
        Session.set(IS_SHOW_UPGRADE_SESSION_KEY, false)
    } else if (Session.get(IS_SHOW_UPGRADE_SESSION_KEY) === false) {
        ElNotification({
            title: '新版本升级',
            message: "检测到系统新版本，前端页面更新...",
            type: 'success',
            duration: 5000,
            onClose: () => {
