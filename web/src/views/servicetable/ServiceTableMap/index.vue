        ...
        <el-card shadow="always" style="width: 100%;">
          <el-collapse v-model="isCollapsed">
            <el-collapse-item name="1" title="更新映射表">
                  ...
                  <el-button type="primary" @click="submitUpload" style="float: right; margin-bottom: 10px">
                    提交
                  </el-button>
            </el-collapse-item>
          </el-collapse>
...
const submitUpload = async () => {
  ...
  await ElMessageBox.confirm(`确定上传 ${fileList.value[0].name} ?`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
        ...
        try {
          ...
          await uploadMap(formData).then((res:any) => {
            if (res.status) {
              ElMessage.success({
                message: res.message,
                duration: 5000,
